...
//define('addr',   isset($_SERVER['SCRIPT_URI']) ? $_SERVER['SCRIPT_URI'] : 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF']);
            
//define( 'addr',  isset( $_SERVER['PATH_INFO'] ) ? str_replace( $_SERVER['PATH_INFO'], '', $_SERVER['SCRIPT_URI'] ) : $_SERVER['SCRIPT_URI'] );
define('addr',BX_WEBROOT.'admin/webinc/openid/');
...
class ConcreteServer extends OpenIDServer {
    ...
    function is_valid($req) {
        if( addr != $req->get('identity').'admin/webinc/openid/' ) {
            bx_helpers_debug::webdump(addr);
            bx_helpers_debug::webdump($req->get('identity').'admin/webinc/openid/');
            return false;
        }
...
switch( $action ) {
    case 'login':
        ...
        else if( $user = $permObj->getUsername() ) {
    ...
    case 'allow':
        if( $_SERVER['REQUEST_METHOD'] == 'POST' ) {
            ...
            if($permObj->isLoggedIn() && BX_WEBROOT == $identity ) {
