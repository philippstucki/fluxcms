...
var webFXTreeHandler = {
    ...
    activateItem    : function(id,state) {
        if (this.activeItemId != 0) {
            activeItem = document.getElementById(this.activeItemId);
            if (activeItem) {
                activeItem.setAttribute('style', 'background-color:'+webFXTreeConfig.itemBgColor+';');
            }
        }
...
function WebFXTreeAbstractNode(sText, sAction, sIconAction, sTitle, sTags) {
	this.childNodes  = [];
...
function WebFXTree(sText, sAction, sBehavior, sIcon, sOpenIcon, sIconAction, sTitle, sTags) {
	this.base = WebFXTreeAbstractNode;
	this.base(sText, sAction, sIconAction);
...
function WebFXTreeItem(sText, sAction, eParent, sIcon, sOpenIcon, sIconAction, sTitle, sTags) {
	this.base = WebFXTreeAbstractNode;
...
WebFXTreeItem.prototype.toString = function (nItem, nItemCount) {
	...
	str += "<img title=\""+this.title+"\" id=\"" + this.id + "-icon\" class=\"webfx-tree-icon\" width=\"18\" height=\"18\" src=\"" + ((webFXTreeHandler.behavior == 'classic' && this.open)?this.openIcon:this.icon) + "\" onclick=\""+this.iconAction + " \" oncontextmenu=\"try{if (event) {event.preventDefault()};"+this.iconAction + ";} catch(e){}\"> <a oncontextmenu=\"try{if (event) {event.preventDefault()};"+this.iconAction + ";} catch(e){}\" target=\"edit\" href=\"" + this.action + "\" title=\""+this.title+"\"  id=\"" + this.id + "-anchor\" onfocus=\"webFXTreeHandler.focus(this);\" onclick=\"webFXTreeHandler.activateItem('"+this.id+"', 1);\" onblur=\"webFXTreeHandler.blur(this);\" href=\"../edit/\">" + label + "</a></div>";
	str += "<div id=\"" + this.id + "-cont\" class=\"webfx-tree-container\" style=\"display: " + ((this.open)?'block':'none') + ";\">";
	for (var i = 0; i < this.childNodes.length; i++) {
		str += this.childNodes[i].toString(i,this.childNodes.length);
	}
