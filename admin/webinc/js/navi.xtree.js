...
function showPopUp (name,id,parentid,view,title,t) {
	...
	if ( t) {
		WebFXTreeObjectId = 	t.id.replace('-icon','');
	}
...
//	webFXTreeHandler.all[WebFXTreeObjectId].expandAll();
	webFXTreeHandler.reload(WebFXTreeObjectId);
...
function insertXref() {
	 showMoveLayers('PopUp'+TableName,'','hide');
	if (parent.edit.focusOnField) {
		replaceTextareaSelection(parent.edit.focusOnField,"<xref linkend=\""+TableName+RecordID+"\"/>","","");
	}
...
function replaceTextareaSelection(inputObj, before, middle, after) {
	if (isMSIEWin ) {
		if(parent.edit.focusOnFieldSelection ) {
			parent.edit.focusOnFieldSelection.text = before + middle + after;
		} else  {
