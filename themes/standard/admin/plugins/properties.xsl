...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns="http://www.w3.org/1999/xhtml" xmlns:i18n="http://apache.org/cocoon/i18n/2.1">
    ...
    <xsl:template match="plugin[@name='admin_properties']">
        ...
        <xsl:for-each select="properties/categories/category[@hasProperties != '']">
            <span>
                ...
                <xsl:value-of select="@name"/></a><xsl:text> </xsl:text>
            </span>
        </xsl:for-each>
            ...
            <p><input type="submit" accesskey="s" name="send" value="Save"/></p>
        </form>
    </xsl:template>
    ...
    <xsl:template name="urlEncodeSlash">
        ...
        <xsl:if test="contains($string, '/')">
            <xsl:value-of select="substring-before($string, '/')"/>%2F<xsl:call-template name="urlEncodeSlash"><xsl:with-param name="string"><xsl:value-of select="substring-after($string,'/')"/></xsl:with-param></xsl:call-template>
        </xsl:if>
    ...
    <xsl:template match="*[@type='datetime']" mode="propertyfields">
        <tr>
            ...
            <td class="blackH5">
                <xsl:variable name="encPath"><xsl:call-template name="urlEncodeSlash"><xsl:with-param name="string"><xsl:value-of select="../../@path"/></xsl:with-param></xsl:call-template></xsl:variable>
            ...
            <a href="#" onClick="cal.select(document.getElementById('{../@fieldname}'),'anchor_{$inpName}','dd.MM.yyyy 00:00:00'); return false;" name="anchor_{$inpName}" id="anchor_{$inpName}">select</a><div id="caldiv" style="position:absolute;visibility:hidden;background-color:white;layer-background-color:white;"></div>
            </td>
        </tr>
    </xsl:template>
    ...
    <xsl:template match="*[@type='readonly']" mode="propertyfields">
        <tr>
            <td><div class="blackH5"><xsl:value-of select="concat(../@namespace, ':', ../@name)"/></div></td>
            <td class="blackH5">
                <xsl:value-of select="../@value"/>
            </td>
        </tr>
