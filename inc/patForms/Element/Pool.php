...
class patForms_Element_Pool extends patForms_Element
{
	...
	*/
	function serializeHtmlDefault( $value )
	{
		...
		// box of candidates
		$element	.=	'<td>';
		$attCollection['id']	=	'candidates_' . $name;
		...
		// box of members
		$element	.=	'<td>';
		$attCollection['id']	=	'members_' . $name;
		...
		$element	.=	"</tr>\n<tr>";
		$element	.=	'<td><a '.$class.'href="javascript:pool_'. $name .'.add();">'. $tooladd.'</a></td>';
		$element	.=	'<td><a '.$class.'href="javascript:pool_'. $name .'.remove();">'. $toolremove.'</a></td>';
		$element	.=	"</tr>\n";
