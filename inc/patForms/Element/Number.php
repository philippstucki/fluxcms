...
class patForms_Element_Number extends patForms_Element
{
	...
	*/
    ...
    {
		// not submitted yet - let's see what we want to do with
		// this element
		if( !$this->submitted )
		{
			// use a default value?
			if( isset( $this->attributes['default'] ) )
			{
				...
				// empty default value: this is a special case
				// to avoid having the number format display,
			...
			// format the number
			$this->attributes['value'] = $this->formatNumber( $value );
			
			// return the value for this case
			return $value;
		}
		
		// store the value in the value attribute... default
		// behavior regardless of whether the element is valid
		// or not.
