 ...
 * 
 * Upload-Element for single file upload. Handles upload and drops 
 ...
 * @author		Gerd Schaufelberger <argh@php-tools.net>
 * @todo		file permission attribute does cannot be casted to octal-number
 * @todo		change html-output if file was already uploaded (use session)
 * @todo		convert permission to octal
...
class patForms_Element_File extends patForms_Element
{
	...
	*/
	...
	var	$validatorErrorCodes  =   array(
		...
		"de" =>	array(
			...
			2	=>	"Dateiname existiert bereits - die Datei kann nicht �berschrieben werden",
			3	=>	"Die Datei ist gr��er als die zugelassene Maximalgr��e von [MAXSIZE].",
		...
		"fr" =>	array(
			...
			2	=>	"Ce fichier existe d�j� - il ne peut pas �tre remplac�.",
			3	=>	"La taille du fichier est plus grande que la maximum autoris� de [MAXSIZE]",
			4	=>	"Le fichier n'a �t� t�l�charg� que partiellement",
	...
	*/
	function validateElement( $value )
	{
		...
		if( isset( $_FILES ) && isset( $_FILES[$nameUpload] ) )
		{
			$uploadFile	=	$_FILES[$nameUpload]['tmp_name'];
			$error		=	$_FILES[$nameUpload]['error'];
			$mime		=	$_FILES[$nameUpload]['type'];			
			$value		=	$this->_replaceChars( $_FILES[$nameUpload]['name'] );
		...
		{
			$tempfile	=	tempnam( $tempdir, $name . '_' );
	...
	*/
    function finalizeElement( $value )
    {
		...
		else if( isset( $_FILES[$nameUpload]["tmp_name"] ) )
		{
			$uploadFile	=	$_FILES[$nameUpload]["tmp_name"];
		}
	...
	function _checkErrorCode( $code )
	{
		switch( $code )
		{
			...
			case	UPLOAD_ERR_FORM_SIZE:
				$this->addValidationError( 3, array( 'maxsize' => ini_get( 'upload_max_filesize' ) ) );
				return false;
				break;
