...
class patForms_Element_Enum extends patForms_Element
{
	...
	function getValues()
	{
		...
		if( isset( $this->attributes["datasource"] ) ) {
			...
			*/
			    ...
			    $static = explode("::",$this->attributes["datasource"] );
			    if( is_callable( $static, false) ){
				    $values	= call_user_func( $static, $this);
			    }
			}			
		}
	...
	* @todo     Fix maxsize=auto when using optgroups
	*/
	function serializeHtmlDefault( $value )
	{
		...
		foreach($values as $line => $optionDef) {
		    // Is it a group?
		    if (isset($optionDef['values'])) {
		        $attribs = array('label' => $optionDef['label']);
		        $element .= $this->createTag('optgroup', 'opening', $attribs);
		        foreach ($optionDef['values'] as $option) {
        			$element .= $this->_createOptionTag($option, $value);
		        }
		        $element .= $this->createTag('optgroup', 'closing');
		    	
		    } else {
    			$element .= $this->_createOptionTag($optionDef, $value);
		    }
    ...
    * @access protected
    * @param  array
    * @param  mixed     the current value of the element
    * @return string
    */	
	function _createOptionTag($optionDef, $value)
	{
		...
		return $this->createTag( "option", "full", $attribs, $optionDef["label"] );
	}
	...
	*/
	function serializeHtmlReadonly( $value )
	{
		...
		$values = $this->getFlatValues($values);
		foreach ($values as $line => $optionDef) {
   ...
   /**
	* get only flat values, without optgroup structure
	*
	* @return array
	*/
	function getFlatValues($options)
	{
	    $flat = array();
	    foreach ($options as $group) {
	        ...
	        $flat = array_merge($flat, $group['values']);
	    }
	    return $flat;
	}
	...
	function toXML ($namespace = null)
	{
		...
		foreach ($options as $opt) {
		    if (isset($opt['values'])) {
		    	$values = $opt['values'];
		    	unset($opt['values']);
		    	$tag .= $this->createTag($groupName, 'opening', $opt);
        		foreach ($options as $opt2) {
                    $tag .= $this->createTag($optName, 'empty', $opt2);
        		}
		    } else {
