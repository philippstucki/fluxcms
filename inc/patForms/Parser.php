 ...
 * @copyright	PHP Application Tools <http://www.php-tools.net>
 * @version		1.0
 * @todo		unified treatment of CData and children of the elements
 *				=> CData must be treated as children. This will allow
 *				injection of dynamic content inside any tag.
 * @todo		implement constructor to get default values from 'static properties'
...
class patForms_Parser
{
	...
	*/
	function parseFile( $filename, $outputFile = null )
	{
		...
		$string	=	$this->file_get_contents( $this->_adjustFilename( $this->_sourceFile ) );
		if( $string === false )
		{
			return patErrorManager::raiseError(
	...
	*/
	function _checkCache()
	{
		...
		$form	=	$this->file_get_contents( $cacheFile );
		if( $form === false )
			return false;
		...
		switch( $name )
		{
				...
				switch( strtolower( $el["attributes"]["type"] ) )
				{
					...
					case	"callback":
						...
						elseif( isset( $el["attributes"]["function"] ) )
							...
							die( "No callback method specified." );
			...
			case	"RadioGroup":
				...
				if( !empty( $template ) ) {
					...
					$el['renderer']->setPlaceholder( '{PATFORMS_ELEMENT_'.strtoupper( $el['attributes']['name'] ).'_%s}' );
				}
				...
				$this->_characterData( $this->_getPlaceholderForElement( $el['attributes']['name'] ) );
				break;
	...
	*/
	function &_callNamespaceHandler( $ns, $name, $attributes, $data )
	{
		if( !method_exists( $this->_namespaceHandlers[$ns], $name ) ) {
			die( "Unknown tag $name in namespace $ns." );
		}
	...
	* @todo		allow more than one element with the same name
	*/
	function addElementDefinition( $name, $type, $attributes, $renderer = null )
	{
		...
		$this->_elementDefinitions[$this->_currentForm][$name]	=	array(
														"type"			=>	$type,
														"attributes"	=>	$attributes,
														"renderer"		=>	&$renderer
										 			);
	...
	*/
	function addElementDefinitionByArray( $def )
	{
		...
		$this->_elementDefinitions[$this->_currentForm][$name] = $def;
		return	true;
	}
	...
	* @todo		fix this method! It is totally broken as patForms_Parser now may contain more than one form. Best solution would be removing the method if nobody objects
	*/
	...
	function &getForm($name = null)
	{
		...
		if ($name === null) {
			...
			foreach ($this->_elementDefinitions as $formName => $elementDefinitions) {
				...
				$this->_form->addForm( patForms::createForm( $elementDefinitions, $this->_formAttributes[$formName] ) );
			}
			return $this->_form;
		}
		...
		$this->_form[$name] = &patForms::createForm( $this->_elementDefinitions[$name], $this->_formAttributes[$name] );
		return	$this->_form[$name];
	}
	...
	*/
	function getHTML()
	{
		...
		$this->_html	=	$this->file_get_contents( $this->_adjustFilename( $this->_outputFile ) );
		if( $this->_html === false )
		{
			return patErrorManager::raiseError(
				PATFORMS_PARSER_ERROR_FILE_NOT_FOUND, 
	...
	*
	* This is needed to make patForms work with PHP < 4.3
	*
	* @access	public
	* @static
	* @param	string	filename
   ...
   /**
	* Replacement for is_a
	*
	* This is needed to make patForms work with PHP < 4.2
	*
	* @access	public
	* @static
	* @param	object	object to test
	* @param	string	classname
	* @return	boolean	true of object is an instance of classname or derived from this class
	*/
	function is_a( $object, $classname )
	{
		...
		if( get_class( $object ) == strtolower( $classname ) )
			return	true;
