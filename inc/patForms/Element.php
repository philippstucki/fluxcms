...
class patForms_Element
{
	...
	*/
    function    addValidationError( $code, $vars = array() )
    {
		...
		while( true )
		{
			...
			{
				$error	=	array( "element" => $element, "code" => $code, "message" => '<i18n:text>'.$this->validatorErrorCodes[$lang][$code].'</i18n:text>' );
				break;
			}
	...
	function createTag( $tagname, $type = "full", $attributes = array(), $value = false )
	{
		switch( $type )
		{
			...
			case "opening":
				...
				foreach( $attributes as $attributeName => $attributeValue )
				{
					$tag	=	$tag . " ".$attributeName."=\"". (string)$attributeValue ."\"";
				}
			...
			case "full":
				...
				return patForms_Element::createTag( $tagname, "opening", $attributes ).htmlspecialchars( $value ).patForms_Element::createTag( $tagname, "closing" );
				break;
		}
	}
	...
	*/
	function applyFilter( &$filter, $outFunc = null )
	{
		$this->filters[] = &$filter;
		return true;
	}
