...
class popoon {
    ...
    function isError($data) {
        return (bool) (is_object($data) && (get_class($data) == 'PEAR_Error' || is_subclass_of($data, 'PEAR_Error')));
    }
    ...
    function handleError($data) {
        if (popoon::isError($data) && $data->getMode() == PEAR_ERROR_RETURN) {
            ...
            if ($data->getCode() & POPOON_ERROR_FATAL & $errorLevel) {
                ...
                if ($data->getCode() & POPOON_ERROR_WARNING & $errorLevel) {
                    print "<b>Warning (Popoon)</b>: " . $data->getMessage() . "<br />";
                } else
                    if ($data->getCode() & POPOON_ERROR_NOTICE & $errorLevel) {
