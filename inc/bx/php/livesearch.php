...
if (strlen($search) > 3) {
     $res = $db->query("select post_uri, post_title from ".$prefix."blogposts as  blogposts where  post_status & $perm and MATCH (post_content,post_title) AGAINST (".$db->quote( $search) .")  LIMIT 20");
     if ($res->numRows() == 0) {
      $res = $db->query("select post_uri, post_title from ".$prefix."blogposts as  blogposts where  post_status & $perm and post_title like '%" . $search . "%' order by post_date DESC LIMIT 20");
     }
     
     if ($res->numRows() == 0 ) {
      $res = $db->query("select post_uri, post_title from ".$prefix."blogposts as  blogposts where  post_status & $perm and post_content like '%" . $search . "%' order by post_date DESC LIMIT 20");
     }
} else {
    $res = $db->query("select post_uri, post_title from ".$prefix."blogposts as  blogposts where post_status & $perm and post_title like '%" . $search . "%' order by post_date DESC LIMIT 20");
