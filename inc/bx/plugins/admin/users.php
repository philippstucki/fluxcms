    ...
    public function handlePost($path, $name, $ext, $data=null) {
		...
		if($re['user_pass'] == md5($data['pwd'])) {
			if(isset($user_id)) {
				if(isset($data['new_pwd']) && $data['new_pwd'] && isset($data['new_pwd_re']) && $data['new_pwd_re']) {
					if($data['new_pwd'] == $data['new_pwd_re']) {
						$query = "update ".$tablePrefix."users SET user_login = '".$data['username']."', user_fullname = '".$data['fullname']."', user_email = '".$data['mail']."', user_gupi = '".$data['gupi']."', user_gid = '".$data['gid']."', user_adminlang = '".$data['lang']."', plazes_username = '".$data['plazes_username']."', plazes_password = '".$data['plazes_pwd']."' , user_pass = '".md5($data['new_pwd'])."' where id = ".$user_id;
					} else {
				...
				if(!isset($query)) {
					$query = "update ".$tablePrefix."users SET user_login = '".$data['username']."', user_fullname = '".$data['fullname']."', user_email = '".$data['mail']."', user_gupi = '".$data['gupi']."', user_gid = '".$data['gid']."', user_adminlang = '".$data['lang']."', plazes_username = '".$data['plazes_username']."', plazes_password = '".$data['plazes_pwd']."' , user_pass = '".$data['new_pwd']."' where id = ".$user_id;		
				}
				if(isset($query)) {
					$res = $GLOBALS['POOL']->db->query($query);
				}
			...
			if(isset($data['add']) && $data['add']) {
				$query = "insert into ".$tablePrefix."users (user_login, user_fullname, user_email, user_gupi, user_gid, user_adminlang, plazes_username, plazes_password, user_pass) value('".$data['username']."', '".$data['fullname']."', '".$data['mail']."', '".$data['gupi']."', '".$data['gid']."', '".$data['lang']."', '".$data['plazes_username']."', '".$data['plazes_pwd']."', '".md5($data['new_pwd'])."')";
